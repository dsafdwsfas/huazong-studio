  ...
  <div class="topbar">
    <nav class="nav">
      ...
      <div class="nav-right">
        <!-- 汉堡菜单按钮（移动端） -->
        <button
          class="hamburger-btn show-on-mobile"
          @click="toggleMobileMenu"
          aria-label="菜单"
        >
          <menu-icon v-if="isMobileMenuHidden" :size="24" />
          <x-icon v-else :size="24" />
        </button>

        <router-link
          class="nav-item hide-on-mobile"
          :to="{
            name: 'checks'
          }"
        ...
        <router-link
          class="nav-item mr05 hide-on-mobile"
          :to="{
            name: 'todos',
        ...
        <global-search-field
          class="flexrow-item mr0 hide-on-mobile"
    ...
    <div
      ...
      @click="toggleMobileMenu()"
      v-if="!isMobileMenuHidden"
    ></div>
    <nav class="mobile-menu" v-if="!isMobileMenuHidden">
      <ul>
        <li v-if="isCurrentUserSupervisor">
          <router-link :to="{ name: 'checks' }" @click="toggleMobileMenu()">
            {{ $t('tasks.my_checks') }}
          </router-link>
        </li>
        <li v-if="!isCurrentUserAdmin && !isCurrentUserClient">
          <router-link
            :to="{ name: 'todos', query: { section: 'todos' } }"
            @click="toggleMobileMenu()"
          >
            {{ $t('tasks.my_tasks') }}
          </router-link>
        ...
        <li v-if="!isCurrentUserAdmin && !isCurrentUserClient">
          <router-link
            :to="{ name: 'todos', query: { section: 'timesheets' } }"
            @click="toggleMobileMenu()"
          >
            {{ $t('timesheets.title') }}
          </router-link>
        ...
        <hr />
        <li>
          <router-link :to="{ name: 'profile' }" @click="toggleMobileMenu()">
            {{ $t('main.profile') }}
          </router-link>
        </li>
        <li @click="toggleDarkTheme(); toggleMobileMenu()">
          {{ !isDarkTheme ? $t('main.dark_theme') : $t('main.white_theme') }}
        </li>
        <hr />
        <li>
          <router-link :to="{ name: 'logout' }" class="flexrow" @click="toggleMobileMenu()">
            <log-out-icon class="flexrow-item icon-1x" />
            <span class="flexrow-item">{{ $t('main.logout') }}</span>
          </router-link>
        </li>
  ...
  computed: {
    ...
    sectionOptions() {
      ...
      if (!this.isCurrentUserClient && isNotOnlyAssets) {
        options.push({ label: '分镜面板', value: 'storyboard' })
      }
      if (!this.isCurrentUserClient && isNotOnlyAssets) {
      ...
      if (!this.isCurrentUserClient) {
        ...
        options.push({ label: '仪表盘', value: 'production-dashboard' })
        options.push({ label: '权限管理', value: 'production-permissions' })
  ...
  methods: {
    ...
    episodifyRoute(route, section, episodeId, isTVShow) {
      const isEpisodeContext =
        ...
        section !== 'production-permissions' &&
        section !== 'episodes'
      if (isEpisodeContext) {
