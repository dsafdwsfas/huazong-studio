<template>
  <div :class="{ theme: true, dark: isDarkTheme }">
    ...
    <global-drop-zone @files-dropped="onGlobalFilesDropped" />
    ...
    <cos-upload-progress
      ...
      @toggle-pause-all="onTogglePauseAll"
...
import COSUploadProgress from '@/components/widgets/COSUploadProgress.vue'
import GlobalDropZone from '@/components/widgets/GlobalDropZone.vue'
...
export default {
  ...
  methods: {
    ...
    onGlobalFilesDropped(files) {
      this.addFilesToQueue({ files, meta: { type: 'preview' } })
    },

    onTogglePause(itemId) {
      const item = this.uploadItems.find(i => i.id === itemId)
    ...
    onCancelUpload(itemId) {
      this.cancelUpload(itemId)
    },

    onTogglePauseAll() {
      const active = this.uploadItems.filter(
        i => i.status !== 'complete' && i.status !== 'error'
      )
      const allPaused = active.every(i => i.status === 'paused')
      if (allPaused) {
