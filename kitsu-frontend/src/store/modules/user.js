import permissionsApi from '@/store/api/permissions'
...
const initialState = {
  ...
  // 项目角色缓存: { projectId: "director" | "producer" | "artist" | "observer" }
  projectRoles: {},
  // 实体访问控制缓存: { projectId: [{ entity_id, project_role, can_view, can_edit, can_comment }] }
  entityAccessControls: {}
}
...
const getters = {
  ...
  currentProjectRole: state => projectId => state.projectRoles[projectId] || null,
  isProjectDirector: state => projectId =>
    state.projectRoles[projectId] === 'director',
  isProjectProducer: state => projectId =>
    state.projectRoles[projectId] === 'producer',
  isProjectArtist: state => projectId =>
    state.projectRoles[projectId] === 'artist',
  isProjectObserver: state => projectId =>
    state.projectRoles[projectId] === 'observer',
  canEditInProject: (state, getters) => projectId => {
    if (getters.isCurrentUserAdmin) return true
    const role = state.projectRoles[projectId]
...
const actions = {
  ...
  async loadProjectRole({ commit, state }, projectId) {
    if (!projectId || !state.user) return
    try {
      const roles = await permissionsApi.getProjectTeamRoles(projectId)
      const myRole = roles.find(r => r.person_id === state.user.id)
      commit('SET_PROJECT_ROLE', {
        projectId,
        role: myRole?.project_role || null
      })
    } catch (err) {
      console.warn('Failed to load project role:', err)
    }
  },

  async setMemberProjectRole({ commit }, { projectId, personId, projectRole }) {
    await permissionsApi.setMemberRole(projectId, personId, projectRole)
  },

  async loadAccessControls({ commit }, projectId) {
    const acls = await permissionsApi.getAccessControls(projectId)
    commit('SET_ENTITY_ACCESS_CONTROLS', { projectId, acls })
    return acls
  },

  async setAccessControl({ commit }, { projectId, data }) {
    return permissionsApi.setAccessControl(projectId, data)
  },

  async deleteAccessControl({ commit }, { projectId, aclId }) {
    return permissionsApi.deleteAccessControl(projectId, aclId)
  },
...
const mutations = {
  ...
  SET_PROJECT_ROLE(state, { projectId, role }) {
    state.projectRoles = {
      ...state.projectRoles,
      [projectId]: role
    }
  ...
  SET_ENTITY_ACCESS_CONTROLS(state, { projectId, acls }) {
    state.entityAccessControls = {
      ...state.entityAccessControls,
      [projectId]: acls
    }
  }
