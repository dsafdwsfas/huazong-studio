...
# 项目内角色（三级权限体系中的第二级）
PROJECT_ROLE_TYPES = [
    ("director", "导演"),          # 项目负责人，可编辑所有内容
    ("producer", "制片"),          # 制片主任，管理进度和资源
    ("artist", "美术"),            # 执行美术工作，可编辑分配的资产
    ("observer", "观察者"),        # 只读权限，无编辑权
]
...
class ProjectPersonLink(db.Model):
    ...
    project_role = db.Column(
        ChoiceType(PROJECT_ROLE_TYPES),
        default="artist",
        nullable=True,  # nullable 兼容旧数据
    )
    shotgun_id = db.Column(db.Integer)


class EntityAccessControl(db.Model, BaseMixin, SerializerMixin):
    """
    实体级访问控制（三级权限体系中的第三级）。
    控制特定项目角色对特定实体的访问权限。
    """
    __tablename__ = "entity_access_control"
    entity_id = db.Column(
        UUIDType(binary=False),
    ...
    __table_args__ = (
        db.UniqueConstraint(
            "entity_id", "project_id", "project_role",
            name="entity_access_uc"
        ),
    )
