...
def _init_task_types_for_domain(domain):
    ...
    if domain == "2d":
        ...
        fx = tasks_service.get_or_create_department("特效", "#26C6DA")
        compositing = tasks_service.get_or_create_department(
            "合成", "#F06292"
        )
        concept = tasks_service.get_or_create_department("概念", "#8D6E63")
        layout = tasks_service.get_or_create_department("布局", "#7CB342")

        tasks_service.get_or_create_task_type(concept, "概念设计", "#8D6E63", 1)
        tasks_service.get_or_create_task_type(
            modeling, "建模", "#78909C", 2
        )
        tasks_service.get_or_create_task_type(
            modeling, "材质渲染", "#64B5F6", 3
        )
        tasks_service.get_or_create_task_type(
            animation, "绑定", "#9CCC65", 4
        )
        ...
        tasks_service.get_or_create_task_type(
            fx, "特效", "#26C6DA", priority=5, for_entity="Shot"
        )
        tasks_service.get_or_create_task_type(
            compositing,
            "渲染",
        ...
        tasks_service.get_or_create_task_type(
            concept, "概念设计", "#8D6E63", 1, for_entity="Concept"
        )
...
def init_data(domain="3d"):
    ...
    with app.app_context():
        ...
        tasks_service.get_or_create_status(
            "制作中", "wip", "#3273dc", is_wip=True
        )
        tasks_service.get_or_create_status(
            "等待审核", "wfa", "#ab26ff", is_feedback_request=True
        )
        tasks_service.get_or_create_status(
            "返工", "retake", "#ff3860", is_retake=True
        )
        tasks_service.get_or_create_status(
            "已完成", "done", "#22d160", is_done=True
        )
        tasks_service.get_or_create_status(
            "准备就绪", "ready", "#fbc02d"
        )
